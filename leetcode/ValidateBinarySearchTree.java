// ~18 min had to look at the prev answer. but why long would work, but not int?
/**
 * Definition for a binary tree node.
 * public class TreeNode {
 *     int val;
 *     TreeNode left;
 *     TreeNode right;
 *     TreeNode() {}
 *     TreeNode(int val) { this.val = val; }
 *     TreeNode(int val, TreeNode left, TreeNode right) {
 *         this.val = val;
 *         this.left = left;
 *         this.right = right;
 *     }
 * }
 */
class Solution {
    public boolean isValidBST(TreeNode root) {
        return checker(root, Long.MIN_VALUE, Long.MAX_VALUE);
    }

    public boolean checker(TreeNode root, long min, long max) {
        if (root == null) return true;
        if ((long) root.val <= min || (long) root.val >= max) return false;
        return checker(root.left, min, (long) root.val) && checker(root.right, (long) root.val, max);
    }
}
